...
4. Upload the sponsor logos into the `sponsor-logos` bucket. `SUPABASE_URL=... SUPABASE_SERVICE_ROLE_KEY=... npm run upload:sponsor-logos` uploads everything in `public/sponsors/`; `--dry-run` lists the files first. The dashboard's storage uploader works too. The service-role key is server-only: keep it out of the repository, out of `.env.local`, and never give it a `VITE_` prefix.
