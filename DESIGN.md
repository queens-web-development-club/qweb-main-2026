...
September 2026 polish: Team entries without a supplied photo use `/assets/Unknown_Member.jpg` in the same portrait frame as supplied photos. Portraits use a 4:5 ratio at every width. On desktop, co-chair and executive portraits share one card width (`--card-gap` 20px between cards, `--group-gap` 48px between the two groups, 12px/32px below 800px); between `701px` and `999px` the groups stack with five equal columns each, and at `700px` and below each person is a single row with a 96px portrait floated beside the text. Join uses an open, ruled invitation with rectangular actions using `--radius`, followed by unboxed contact columns (stacked with dividers below 700px). Sponsor statistics use three unboxed columns at both desktop and mobile widths, with `--t-stat` values. These refinements supersede the earlier card descriptions below; typography and the underlying palette are unchanged.
...
- Team places the co-chair and executive groups side by side in one row of seven equal-width cards (two co-chair columns, five executive columns in `.team-people`) so every exec fits the row within the 1130px container. Below `1000px` the groups stack (five columns each), and at `700px` and below people are listed one per row. Join uses a two-sided flex composition plus three route cards that stack on mobile. Sponsors use a five-column logo grid that becomes two columns on mobile.
