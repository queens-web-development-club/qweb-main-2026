...
| Section heading scale | `clamp(42px, 5.45vw, 70px)` About; `clamp(34px, 4.3vw, 56px)` shared headings | `AboutUs.css`, `SectionHeading.css` |
...
- Major sections use approximately `102px`–`112px` top and `82px`–`118px` bottom padding; About uses `112px`/`118px`, while compact mobile sections reduce to `76px`/`82px`.
...
| `Projects` | Presents eleven member-built project treatments, linking out where a live site is known | `src/pages/Projects/Projects.tsx` and `Projects.css` |
...
| `Team` / `Person` | Presents co-chairs and executives with optional personal details | `src/pages/Team/Team.tsx` and `Team.css` |
...
- The site is a single React route composed by `Landing` in `src/main.tsx`, with four anchored regions: `#home` is the landing frame, `#about` groups About Us, Team, and Sponsors, `#education` groups Education, Term, and Projects, and `#join` contains membership routes.
...
- Projects use a three-column grid and keep all project cards visible on small screens so the migrated project list remains discoverable.
...
- Team uses a two-card co-chair grid and four-column executive grid. Join uses a two-sided flex composition plus three route cards that stack on mobile. Sponsors use a five-column logo grid that becomes two columns on mobile.
...
| About offering focus | Scroll position while About is in view | `620ms`, opacity/translateY/scale transition on the offering nearest the viewport center | `AboutUs.tsx` and `AboutUs.css`, `.about-us__offering` |
