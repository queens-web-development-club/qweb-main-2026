...
- Team places the co-chair and executive groups side by side in one row of seven equal-width cards (two co-chair columns, five executive columns in `.team-people`) so every exec fits the row within the 1130px container; the groups stack below `700px`. Join uses a two-sided flex composition plus three route cards that stack on mobile. Sponsors use a five-column logo grid that becomes two columns on mobile.
