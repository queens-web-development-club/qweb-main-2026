...
- [ ] **Restrict administration and unused services.** *Partly attested 2026-09-07:* MFA is enabled and there are at least two accountable maintainers. *Remaining to close:* confirm former executives have been removed from the GitHub org and Supabase project, that no shared password or stale access token remains, and that Supabase public signup and unused auth providers are off (the site uses none); then write down the annual handover and recovery steps, which do not exist yet.

  **This gate is already failing, and it blocked a launch task on 2026-09-07.** The club's domain `qweb.dev` and the Vercel project still serving the 2024 site are on an account the current co-chair cannot reach: `vercel domains inspect qweb.dev` returns *"You don't have access to the domain qweb.dev under zacfink"*, and that account holds zero projects and zero domains. The nameservers are `ns1/ns2.vercel-dns.com`, so DNS is inside that inaccessible account too. **Nobody currently running QWEB can change where the club's domain points.** Until this is resolved the club cannot move the domain, cannot renew it if the card on file expires, and cannot recover it if the account is closed. It is now the single highest-priority item on this checklist, ahead of anything cosmetic.
...
- [ ] **Perform browser QA on the exact release candidate.** *Known issue to measure, 2026-09-07:* the four team portraits total **4.5 MB** — `Aun.jpeg` is 3.3 MB on its own — for images rendered in a 136px-tall grid. Supabase image transformation is not enabled on this project's plan, so the stored files are what visitors download. Resized copies at 164 KB total were prepared and the content lead chose not to upload them for now. Measure this on a real mobile connection before launch; it is roughly a five-second stall on a slow 3G link and it affects the section most likely to be shared. Test desktop/mobile Chrome, Firefox, and Safari as available; all navigation, external links, contact actions, project scrolling, icon assembly, timeline motion, forms if any, and accessibility states. Inspect console and network responses for unintended data and tracking. Check media sizes and performance under a realistic mobile connection. Record screenshots/results privately when they contain personal data.
...
### Team section verified live (2026-09-07)

Six cards render from the database: Zac and Farhaan as co-chairs, then Griffin (Outreach), Aun (Finance), Alex (Development) and Che (Education). Four portraits resolve to the `team-photos` bucket and return 200; Alex and Che have no file uploaded yet, so their cards fall back to the placeholder portrait rather than a broken image.

First names only, by the content lead's choice. The `role` column was opened to free text so `Finance` could be recorded honestly rather than forced into one of five hardcoded values.

**A note on the domain, because it nearly went wrong.** The CNAME branch was merged ahead of the DNS change on 2026-09-07. That published `qweb.dev` as the Pages custom domain while its A records still resolved to Vercel, which makes GitHub redirect the working URL to a domain serving the old 2024 site. It was reverted within about a minute. The lesson worth keeping: **the CNAME file and the DNS records have to change together**, and the file must go second.
